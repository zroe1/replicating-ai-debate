'''
Code to train the four pixel model in order to replicate "AI safety via debate" 
...
# saved models
PATHS_FOR_SAVED_MODELS = ['four_pxl_judge_NO_REPEAT_Xs.h5', 'models_keras_format/four_pxl_judge_NO_REPEAT_Xs.keras']
...
def generate_training():
  while True:
    ...
    for i in range(128):
      x_train_transformed[i] = apply_mask_to_img(x_train[start+i], 4)
...
for i in range(len(x_test)):
  x_test_transformed[i] = apply_mask_to_img(x_test[i], 4)

for i in range(28):
    print(x_test_transformed[0, i])
...
model.fit(train_dataset, batch_size=batch_size, steps_per_epoch=50000, epochs=1)
